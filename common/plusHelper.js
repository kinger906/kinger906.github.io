const plusHelper = {
    ...
    getCurrentPosition: (callbackSuccess, callbackError) => {
        plus.geolocation.getCurrentPosition(callbackSuccess, callbackError, { geocode: false });
    },
    /**
     * 拍照
     * @param {*} callbackSuccess 成功回调
    ...
    getCamera: (callbackSuccess, callbackError) => {
        ...
        }, callbackError, { filename: '_doc/camera/', index: 1 });
    },
    /**
     * 从相册中选择图片
     * @param {*} callbackSuccess 成功回调
    ...
    chooseImage: (callbackSuccess, callbackError, options = {}) => {
        plus.gallery.pick(callbackSuccess, callbackError, { filter: 'image', multiple: true, system: false, ...options });
    },
    /**
     * 拨打电话
